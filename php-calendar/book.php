...
if (isset($_POST['submit'])) {
    ...
    if ($stmt->execute()) {
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $mysqli->prepare("INSERT INTO bookings (name, timeslot, email, date, vet, petname, contact) VALUES (?,?,?,?,?,?,?)");
            $stmt->bind_param('sssssss', $name, $timeslot, $email, $date, $vet, $petname, $contact);
            $stmt->execute();
